...
# comma-separated origin here needs gcloud's `^#^` custom-delimiter syntax below
# instead of the plain comma-joined form every other var uses (docs/DECISIONS.md
# #128 -- found live: the analogous CI step silently dropped this the same way
# before switching delimiters, once this value actually became multi-valued).
CORS_ALLOWED_ORIGINS ?= http://localhost:5173,https://web-492372502792.us-central1.run.app
# Phase 10 (docs/DECISIONS.md #125): DASHBOARD_DEMO_OPEN_ACCESS-gated judge role.
# Off by default -- pass DASHBOARD_DEMO_OPEN_ACCESS=true explicitly to deploy the
# open-access judge identity this project's own live hackathon demo uses.
DASHBOARD_DEMO_OPEN_ACCESS ?=
