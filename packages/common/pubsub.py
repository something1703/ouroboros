...
def publish_json(topic_id: str, payload: dict[str, Any]) -> str:
    """Publish `payload` as JSON bytes to `topic_id` in the current project. Returns the
    published message ID (blocks on the publish future — ingest runs are already
    request-scoped, so there's no benefit to fire-and-forget here)."""
